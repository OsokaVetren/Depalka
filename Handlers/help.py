...
async def get_help(callback: types.CallbackQuery):
    await callback.message.edit_text("В депалке есть множество способов поднять деняк и стать самым крутым в садике😎\n\n"
                                  ...
                                  "P.S.: напиши /stats, чтобы посмотреть статку последних 5 игр, /top, чтобы увидеть 10 мегазадротов, или /pravda, чтобы узнать секрет🤫",
